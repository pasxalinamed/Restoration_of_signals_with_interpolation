%Tests Gibbs sampling, EM, ML on an acapella song of 4 sec.
...
%audio signal
[x_initial,fs] = audioread('kalanta_4sec.wav');
x_initial=x_initial';
...
audiowrite('kalantaDamaged.wav',full(x_damaged),fs);
%%
...
xlabel('i');
ylabel('x_i');
audiowrite('kalantaGibbrest.wav',x_restoredgibb,fs);

print -deps kalantarestored_song_Gibbs
%%
...
xlabel('i');
ylabel('x_i');
audiowrite('kalantaEMrest.wav',x_restored_EM,fs);
print -deps kalantarestored_song_EM
%%
...
xlabel('i');
ylabel('x_i');
audiowrite('kalantaMLest.wav',x_restored_ML,fs);
print -deps kalantarestored_song_ML
save kalanta